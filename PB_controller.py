...
from controller import MLP,MLPS
...
class PerfBoostController(nn.Module):
    ...
    """
    def __init__(self, noiseless_forward, input_init: torch.Tensor, b_output_init: torch.Tensor,c_output_init: torch.Tensor,
        ...
        self.b_output_init = b_output_init.reshape(1, -1)
        self.c_output_init = c_output_init.reshape(1, -1)
        # set dimensions
        self.dim_in = self.input_init.shape[-1]
        self.dim_out = self.b_output_init.shape[-1]

        # define the REN
        self.M2b = ContractiveREN(
        ...
        self.Mib = MLP(self.dim_in*2,dim_out=self.dim_out)
        self.Mic = MLPS(self.dim_in*2, dim_out=self.dim_out)
    ...
    def reset(self):
        ...
        self.last_binary_output = self.b_output_init.detach().clone()
        self.last_continuous_output = self.c_output_init.detach().clone()


        self.M2b.x = self.M2b.init_x 
        self.M2c.x = self.M2c.init_x    # reset the REN state to the initial value

    def forward(self, x, tau=1.0, hard=False, neural = True):
        """
        ...
        input = torch.cat((x,w_), dim=2)

        out_range = self.Mic(input)*torch.sigmoid(self.M2c(w_))
        out_range = -0.4 + 2 * out_range  # Scale to [0.5, 5.0]

        logits = self.Mib(input)  # shape: [batch, 1]
        u = torch.rand_like(logits)
        gumbel_noise = -torch.log(-torch.log(u + 1e-10) + 1e-10)
        y_soft_i = torch.sigmoid((logits + gumbel_noise) / tau)

        logits2 = self.M2b(w_)  # shape: [batch, 1]
        u2 = torch.rand_like(logits2)
        sign_logit2 = torch.sign(logits2)
        
        gumbel_noise2 = -torch.log(-torch.log(u2 + 1e-10) + 1e-10)
        y_soft_2 = torch.sigmoid((1/(logits2+sign_logit2*1e-10) + gumbel_noise2) / tau)

        y_soft = y_soft_i*y_soft_2

        #output = torch.clamp(output,min = -10,max = 10)
        # update internal states
        self.last_input = x
        
        self.last_continuous_output = out_range
        ...
        if hard:
            ...
            self.last_binary_output = binary_out
            return out_range, binary_out
        #*((y_hard - y_soft).detach() + y_soft)
        else:
            self.last_binary_output = y_soft
